...
export default function InterestsScreen() {
    ...
    const isFinished = currentIndex >= Math.min(SHUFFLED_INTERESTS.length, MAX_ONBOARDING_CARDS);
        ...
        return (
            <View style={styles.container}>
                <View style={styles.content}>
                    <Ionicons name="checkmark-circle" size={80} color="#10B981" style={{ alignSelf: 'center', marginBottom: 24 }} />
                    <Text style={styles.titleCentred}>Harika!</Text>
                    <Text style={styles.subtitleCentred}>{selectedInterests.length} ilgi alanı seçtin. Bunları daha sonra profilinden düzenleyebilirsin.</Text>
                    ...
                    <View style={styles.chipsContainer}>
                        ...
                        {selectedInterests.length > 8 && <Text style={{ color: '#64748B', alignSelf: 'center' }}>+ {selectedInterests.length - 8} daha</Text>}
                    </View>
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.subtitle}>İlgi alanlarına göre etkinlikler bulalım.</Text>
                <Text style={styles.counter}>{currentIndex + 1} / {MAX_ONBOARDING_CARDS}</Text>
            </View>

            <View style={styles.cardContainer}>
                {/* Render the next card beneath to prevent blank flash */}
                {currentIndex + 1 < MAX_ONBOARDING_CARDS && (
                    <View style={[styles.card, { position: 'absolute', transform: [{ scale: 0.95 }], top: 60, opacity: 0.5 }]}>
                        <Text style={styles.cardText}>{SHUFFLED_INTERESTS[currentIndex + 1]}</Text>
                    </View>
                )}

                <Animated.View
                    {...panResponder.panHandlers}
                    style={[getCardStyle(), styles.card, { position: 'absolute', top: 40 }]}
                >
                    ...
                    <Text style={styles.cardText}>{SHUFFLED_INTERESTS[currentIndex]}</Text>
                </Animated.View>
            </View>
...
const styles = StyleSheet.create({
    ...
    titleCentred: { fontSize: 32, fontWeight: '900', color: '#F8FAFC', textAlign: 'center', marginBottom: 16 },
    subtitleCentred: { fontSize: 16, color: '#94A3B8', textAlign: 'center', lineHeight: 24, paddingHorizontal: 20 },
    ...
    cardText: { fontSize: 36, fontWeight: '800', color: '#F8FAFC', textAlign: 'center', padding: 20 },

    badge: { position: 'absolute', top: 40, paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8, borderWidth: 3 },
    likeBadge: { left: 40, borderColor: '#10B981', transform: [{ rotate: '-15deg' }] },
    nopeBadge: { right: 40, borderColor: '#EF4444', transform: [{ rotate: '15deg' }] },
    likeBadgeText: { color: '#10B981', fontSize: 32, fontWeight: '900', letterSpacing: 2 },
    ...
    button: {
        backgroundColor: '#3B82F6', height: 60, borderRadius: 16,
        justifyContent: 'center', alignItems: 'center',
        shadowColor: '#3B82F6', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 8
    },
