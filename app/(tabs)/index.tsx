...
export default function HomeScreen() {
  ...
  const [activeTab, setActiveTab] = useState<"friends" | "my" | "nearby">(
    "friends",
  );
    ...
    try {
      ...
      if (activeTab === "nearby" && user) {
        try {
          const {
            UserManager,
          } = require("../../src/core/identity/UserManager");
          const profileData = await UserManager.getInstance().getUserProfile(
            user.id,
          );
          ...
          if (!loc)
            showToast(
              "Please set your location in Profile > Event Settings",
              "error",
            );
        } catch (e) {
      ...
      const nearbyRes = await eventController.getEvents(
        activeTab === "nearby" && loc
          ? { category: "All", location: loc, radius, userId: user.id }
    ...
    if (styleType === "friends") {
      return (
        ...
        >
          ...
          <Text style={styles.friendEventTitle} numberOfLines={2}>
            {item.title}
          </Text>
          ...
          <View style={styles.friendEventDetailsBox}>
            <View style={styles.eventMetaRow}>
              ...
              <Text
                style={[styles.eventMetaText, { color: theme.textPrimary }]}
              >
                {item.time}
              </Text>
            </View>
      ...
      >
        ...
        <View style={styles.segmentContainer}>
          <TouchableOpacity
            style={[
              styles.segmentBtn,
              activeTab === "friends" && styles.segmentBtnActive,
            ]}
          ...
          <TouchableOpacity
            style={[
              styles.segmentBtn,
              activeTab === "nearby" && styles.segmentBtnActive,
            ]}
          ...
          >
            ...
            <Text
              style={[
                styles.segmentText,
                activeTab === "nearby" && styles.segmentTextActive,
              ]}
            >
              Nearby
            </Text>
              ...
              {RADIUS_OPTIONS.map((opt) => (
                ...
                >
                  <Text
                    style={[
                      styles.radiusText,
                      radius === opt && styles.radiusTextActive,
                    ]}
                  >
                    {opt} km
                  </Text>
        ...
        <View style={styles.verticalFeedContainer}>
          {activeTab === "friends" &&
            (friendsEvents.length > 0 ? (
              friendsEvents.map((item) => (
                <View key={item.id} style={{ marginBottom: 16 }}>
                  {renderEventCard(item, "friends")}
                </View>
              ))
            ) : (
              <Text style={styles.emptyStateText}>
                No friend events right now.
              </Text>
            ))}
